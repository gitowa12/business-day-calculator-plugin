...
export const InputList = ({
  ...
  return (
    <div
      key={row.id}
      className="  border-2  border-gray-300 border-l-8 border-l-blue-500 rounded-lg px-4 py-2 flex items-end w-fit min-w-[950px]"
    >
      <input
        id="srcField"
        ref={inputRef}
      ...
      {isReadOnly ? (
        <button
          type="button" // 同上
          className="border-2 border-yellow-600 rounded text-yellow-600 px-1 py-0.5 mr-2"
          onClick={handleEdit}
        >
          Edit
        </button>
        ...
        <button
          type="button"
          className="border-2 border-green-600 rounded text-green-600 px-1 py-0.5 mr-2"
          onClick={() => {
            handleSave(index);
          }}
        >
      ...
      <button
        type="button" // 同上
        className="border-2 border-red-700 rounded text-red-700 px-1 py-0.5"
